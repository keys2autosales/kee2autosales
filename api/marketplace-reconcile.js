...
function coreModel(modelTrim){
  const raw=String(modelTrim||'').trim();
  const primary=raw.split(/[\/|]/)[0]||raw;
  return norm(primary);
}
function trimText(modelTrim){
  const raw=String(modelTrim||'').trim();
...
function modelScore(identity,v){
  const model=coreModel(v.model_trim);
  if(!model) return {score:0,reason:'no model'};
...
function priceScore(identity,v){
  const lp=identity.price, vp=num(v.fb_price)||num(v.dealer_price);
  if(!lp||!vp) return {score:0,diff:null};
  const diff=Math.abs(lp-vp);
  const pct=diff/Math.max(vp,1);
...
function scoreVehicle(identity,v){
  ...
  const vy=Number(v.year)||0;
  if(identity.year&&vy===identity.year){score+=30;why.push('year');}
  else if(identity.year&&vy&&vy!==identity.year)return {score:-100,why:['year mismatch']};

  const vm=canonToken(norm(v.make));
  if(identity.make&&vm===identity.make){score+=25;why.push('make');}
  else if(identity.make&&vm&&vm!==identity.make)return {score:-100,why:['make mismatch']};

  const ms=modelScore(identity,v);score+=ms.score;if(ms.score>0)why.push(ms.reason);
  else return {score:-50,why:[ms.reason]};

  const trim=trimText(v.model_trim);
  if(trim){const trimTokens=tokens(trim).filter(t=>!STOP.has(t));if(trimTokens.some(t=>identity.tokens.has(t))){score+=5;why.push('trim');}}
  ...
  const inv=norm(v.inventory_status);
  if(/available|active|in stock/.test(inv)){score+=3;why.push('available');}
  else if(/sold|removed|unavailable/.test(inv)){score-=20;why.push('not available');}
...
export default async function handler(req,res){
  ...
  try{
    ...
    return res.status(200).json({ok:true,dry_run:dryRun,total:listings.length,matched,ambiguous,unmatched,with_real_url:withRealUrl,without_real_url:listings.length-withRealUrl,confidence,results});
