...
const keyOf=v=>clean(v.vin).toUpperCase()||`stock:${clean(v.stock_number).toLowerCase()}`;
...
export default async function handler(req,res){
  ...
  try{
    ...
    const snapRows=await sb('inventory_snapshots',{method:'POST',headers:{Prefer:'return=representation'},body:JSON.stringify({source:'DealerSocket IDMS Extension',imported_at:now,row_count:raw.length,notes:`Verified capture ${raw.length}/${expected||raw.length}`})});
    ...
    for(const rawVehicle of raw){
      ...
      const payload={...n,idms_last_seen_at:now,updated_at:now};
      let saved=old;
      if(old){
        const diff=changedFields(old,payload);
        const wasInactive=!['Available','available'].includes(String(old.inventory_status||''));
        ...
        if(wasInactive){returned++;changes.push({snapshot_id:snapshot.id,vehicle_id:saved.id,change_type:'returned',stock_number:n.stock_number,vin:n.vin,vehicle_name:`${n.year||''} ${n.make} ${n.model_trim}`.trim(),changed_fields:diff});}
        else if(Object.keys(diff).length){updated++;changes.push({snapshot_id:snapshot.id,vehicle_id:saved.id,change_type:'updated',stock_number:n.stock_number,vin:n.vin,vehicle_name:`${n.year||''} ${n.make} ${n.model_trim}`.trim(),changed_fields:diff});}
        else unchanged++;
      }else{
        ...
        changes.push({snapshot_id:snapshot.id,vehicle_id:saved?.id||null,change_type:'new',stock_number:n.stock_number,vin:n.vin,vehicle_name:`${n.year||''} ${n.make} ${n.model_trim}`.trim(),changed_fields:{}});
      }
      snapshotItems.push({snapshot_id:snapshot.id,stock_number:n.stock_number,vin:n.vin,year:n.year,make:n.make,model_trim:n.model_trim,color:n.color,mileage:n.mileage,dealer_price:n.dealer_price,photo_count:n.photo_count,inventory_status:n.inventory_status,acquired_date:n.acquired_date,raw_data:rawVehicle});
    }
    ...
    for(const old of current){
      ...
      removed++;
      await sb(`vehicles?id=eq.${encodeURIComponent(old.id)}`,{method:'PATCH',headers:{Prefer:'return=representation'},body:JSON.stringify({inventory_status:'Removed',updated_at:now})});
      changes.push({snapshot_id:snapshot.id,vehicle_id:old.id,change_type:'removed',stock_number:old.stock_number,vin:old.vin,vehicle_name:`${old.year||''} ${old.make||''} ${old.model_trim||''}`.trim(),changed_fields:{inventory_status:{from:old.inventory_status,to:'Removed'}}});
    }
