...
(function(){
  ...
  function rowToVehicle(row,existing){
    ...
    return {
      ...
      color:String(val(row,['exterior_color','color','Color','Exterior Color','Ext Color','Exterior'])||existing?.color||'').trim(),
      interiorColor:String(val(row,['interior_color','Interior Color','Int Color','Interior','Interior Colour'])||existing?.interiorColor||'').trim(),
      bodyStyle:String(val(row,['body_style','Body Style','BodyStyle','Body Type','Vehicle Type'])||existing?.bodyStyle||'').trim(),
      condition:String(val(row,['vehicle_condition','Condition','Vehicle Condition'])||existing?.condition||'').trim(),
      fuelType:String(val(row,['fuel_type','Fuel Type','Fuel','FuelType'])||existing?.fuelType||'').trim(),
      transmission:String(val(row,['transmission','Transmission','Trans','Transmission Type'])||existing?.transmission||'').trim(),
